...
let rec eval env ((cstack, stack, ((st, i, o, r) as c)) as conf) prg = 
        ...
        | p :: ps -> match p with
		   ...
		   | CALL (funcName, _, _) -> eval env ((ps, st)::cstack, stack, c) (env#labeled funcName) (* now has 3 args *)
		   | BEGIN(_, args, locals) -> let c' = State.enter st (args @ locals) in
                                    eval env (addArgs args (cstack, stack, (c', i, o, r))) ps (* some function names were changed*) (* now has 3 args *)
...
let compile (defs, p) =
 let rec compile' = function
  ...
  | Expr.Call (func, args) -> List.concat (List.map compile' (List.rev args)) @ [ CALL (func, List.length args, true)]
  in
  ...
  | Stmt.If (c, tr, fl) -> let elsel = label_maker#nextLabel in let endl = label_maker#nextLabel in
						   ...
						   @ compile'' fl
						   @ [LABEL endl]
  | Stmt.Call (funcName, vars) ->  compile' (Expr.Call (funcName, vars)) (*(List.concat (List.map compile' vars)) @ [CALL (funcName, List.length args, true)]*)
  ...
  | Stmt.Return (Some r) -> (compile' r) @ [RET true]
in
let rec compileF(funcName, (args, locals, body)) = 
    [LABEL funcName; BEGIN (funcName, args, locals)] (* added first parameter*)
    @ compile'' body 
    @ [END]
in
 compile'' p @ [END] @ List.concat (List.map compileF defs)
